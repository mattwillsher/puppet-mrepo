  ...
  case $ensure {
    present: {
      ...
      exec { "Generate mrepo repo ${name}":
        ...
        cwd       => $mrepo::params::src_root,
        path      => [ '/usr/bin', '/bin' ],
        user      => $user,
        group     => $group,
        creates   => $www_root_subdir,
        timeout   => $gen_timeout,
        require   => [ Class['mrepo'], Class['mrepo::package'] ],
      ...
      case $update {
        ...
        default: {
          fail { "Unknown mrepo::repo update value of ${update}": }
        }
      }
    }
    absent: {
    ...
    default: {
      fail { "mrepo::repo ensure needs to be present or absent, not ${ensure}": }
    }
  }
}
